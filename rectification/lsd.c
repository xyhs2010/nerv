                           ...
                           int ** reg_img, int * reg_x, int * reg_y )
{
  ...
  double quant = 1;       /* Bound to the quantization error on the
                                gradient norm.                                */
  double ang_th = 30;     /* Gradient angle tolerance in degrees.           */
  double log_eps = 0.0;     /* Detection threshold: -log10(NFA) > log_eps     */
  double density_th = 0.4;  /* Minimal density of region points in rectangle. */
...
double * yd_lsd_scale_region( int * n_out,
                           double * img, int X, int Y, double density_th )
{
  /* LSD parameters */
  double sigma_scale = 0.6; /* Sigma for Gaussian filter is computed as
                                sigma = sigma_scale/scale.                    */
